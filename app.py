...
# FIX: Point to the actual outputs path generated by your predict.py script
...
# Fallback: If predictions CSV doesn't exist, generate them dynamically using predict logic
    ...
    if model is not None and scaler is not None:
        ...
        for i in range(72):
            ...
            fallback_rows.append({
                "temperature_2m": 25.0 + (5 * np.sin(2 * np.pi * (hour - 6) / 24)) + (day_index * 1.5),
                "relative_humidity_2m": 60.0 - (day_index * 3.0),
                "precipitation": 0.0,
                "surface_pressure": 1010.0,
                "wind_speed_10m": 5.0 + (day_index * 0.5),
        ...
        X_scaled = scaler.transform(prediction_df[expected_features])
        prediction_df["predicted_pm2_5"] = [max(0.0, p) for p in model.predict(X_scaled)]
    else:
        st.error("⚠️ Model artifacts or predictions missing. Please run `python src/predict.py`.")
        st.stop()
...
with tab_health:
    ...
    st.markdown("Air pollution—specifically fine particulate matter (PM2.5)—poses serious health risks. Follow these health guidelines based on predicted pollution levels:")
...
with tab_performance:
    ...
    features = ["temperature_2m", "relative_humidity_2m", "precipitation", "surface_pressure", "wind_speed_10m", "wind_direction_10m", "hour", "day", "day_of_week", "month", "year", "is_weekend"]
    ...
    fig_imp = go.Figure(go.Bar(x=imp_df["Importance"], y=imp_df["Feature"], orientation='h', marker=dict(color="#8F3F97", opacity=0.8)))
    fig_imp.update_layout(height=350, margin=dict(l=20, r=20, t=30, b=20), xaxis_title="Relative Importance Weight", yaxis_title="", plot_bgcolor="rgba(0,0,0,0)")
...
with tab_export:
    ...
    st.download_button(label="⬇️ Download Full 72-Hour Forecast (CSV)", data=csv_data, file_name=f"lahore_aqi_forecast_{datetime.now().strftime('%Y%m%d')}.csv", mime="text/csv")
